...
from src.presentacion.vistas.componentes import (
    MarcoBase, boton_primario, boton_secundario, COLOR_FONDO,
...
class VistaPerfil(MarcoBase):
    def __init__(self, parent, controlador, **kwargs):
        ...
        self._usuario = controlador.usuario_actual
        self._construir()
    ...
    def _construir(self):
        ...
        for i, (etiqueta, valor) in enumerate(campos):
            tk.Label(panel, text=f"{etiqueta}:", bg="white", font=FUENTE_SUBTITULO,
                     anchor=tk.W, width=12).grid(row=i, column=0, padx=15, pady=5, sticky=tk.W)
            tk.Label(panel, text=valor, bg="white", font=FUENTE_NORMAL,
                     anchor=tk.W).grid(row=i, column=1, padx=5, pady=5, sticky=tk.W)

        # Cambiar contraseña
        tk.Label(self, text="Cambiar contraseña", fg=COLOR_PRIMARIO,
                 bg=COLOR_FONDO, font=FUENTE_SUBTITULO).pack(anchor=tk.W, padx=15, pady=(20, 5))
        ...
        self._var_pass1 = tk.StringVar()
        tk.Entry(form, textvariable=self._var_pass1, show="*", width=25,
        ...
        self._var_pass2 = tk.StringVar()
        tk.Entry(form, textvariable=self._var_pass2, show="*", width=25,
                 font=FUENTE_NORMAL).grid(row=1, column=1, padx=10, pady=5)

        self._lbl_msg = tk.Label(self, text="", bg=COLOR_FONDO, font=FUENTE_NORMAL)
        self._lbl_msg.pack(anchor=tk.W, padx=30)

        boton_primario(self, "Cambiar contraseña", self._cambiar_contrasena).pack(
            anchor=tk.W, padx=30, pady=10)

    def _cambiar_contrasena(self):
        p1 = self._var_pass1.get()
        p2 = self._var_pass2.get()
        if p1 != p2:
            self._lbl_msg.config(text="❌ Las contraseñas no coinciden.", fg="#C62828")
            return
        try:
            self._servicio.cambiar_contrasena(self._usuario._id, p1)
            self._lbl_msg.config(text="✅ Contraseña actualizada correctamente.", fg=COLOR_EXITO)
            self._var_pass1.set("")
            self._var_pass2.set("")
