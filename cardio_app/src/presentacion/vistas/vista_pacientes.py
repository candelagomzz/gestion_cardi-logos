...
from src.presentacion.vistas.componentes import (
    ...
    etiqueta_seccion, campo_form, FUENTE_TITULO
)
from src.datos.modelos.entidades import Paciente, GeneroFHIR


class VistaPacientes(MarcoBase):
    def __init__(self, parent, controlador, **kwargs):
        super().__init__(parent, controlador)
        self._servicio = controlador.servicios["pacientes"]
    ...
    def _construir(self):
        ...
        boton_primario(bar, "➕ Nuevo", self._abrir_nuevo).pack(side=tk.LEFT, padx=3)
    ...
    def _cargar_tabla(self):
        ...
        for p in self._servicio.listar():
            self._tabla.insert("", tk.END, iid=p._id, values=(
                p.nombre, p.apellidos, p.genero, p.fecha_nacimiento,
    ...
    def _abrir_nuevo(self):
        _FormularioPaciente(self, self._servicio, None, self._cargar_tabla)

    def _abrir_editar(self):
        if not self._seleccionado_id:
            self.mostrar_info("Selecciona un paciente.")
            return
        paciente = self._servicio.obtener(self._seleccionado_id)
        _FormularioPaciente(self, self._servicio, paciente, self._cargar_tabla)


class _FormularioPaciente(tk.Toplevel):
    def __init__(self, parent, servicio, paciente, callback):
    ...
    def _construir(self):
        ...
        self._var_genero = campo_form(form, "Género (FHIR)", 2, valores=GeneroFHIR.VALORES)
        self._var_nacimiento = campo_form(form, "F. nacimiento (YYYY-MM-DD)", 3)
        self._var_activo = campo_form(form, "Activo", 4, valores=["True", "False"])

        if self._paciente:
            self._var_nombre.set(self._paciente.nombre)
            self._var_apellidos.set(self._paciente.apellidos)
            self._var_genero.set(self._paciente.genero)
            self._var_nacimiento.set(self._paciente.fecha_nacimiento)
            self._var_activo.set(str(self._paciente.activo))
    ...
    def _guardar(self):
        ...
        try:
            if self._es_nuevo:
                ...
                self._servicio.modificar(self._paciente._id, datos)
            self._callback()
            self.destroy()
